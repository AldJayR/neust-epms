...
export async function createProjectFromProposal(
	...
	const created = await db.transaction(async (tx) => {
		...
		await insertAuditLog(
			{
				userId: user.userId,
				action: `Created project ${createdProject.projectId} from proposal ${proposalId}`,
...
export async function linkMoaToProject(
	...
	const [moa] = await db
		...
		.from(moas)
		.where(and(eq(moas.moaId, moaId), isNull(moas.archivedAt)))
		.limit(1);
	...
	await db.transaction(async (tx) => {
		await tx
			.update(projects)
			.set({ moaId, updatedAt: new Date() })
			.where(eq(projects.projectId, projectId));

		await insertAuditLog(
			{
				userId: user.userId,
				action: `Linked MOA ${moaId} to project ${projectId}`,
				tableAffected: "projects",
				ipAddress,
			},
			tx,
