...
- Use a focused test first for each migrated function module; run the relevant test after each move.
...
2. Add a failing test for one project function and one proposal function that stubs `fetch`, invokes the public server function, and asserts the expected URL, authorization header, and returned value.
...
2. Replace local relative session imports with `@/lib/session.server` where the moved location makes them invalid.
3. Export only the public functions needed by routes/components from the relevant feature barrel.
...
2. Keep proposal comments and PDF annotation types with proposals.
3. Merge faculty proposal/project query options into faculty’s existing function module without renaming public exports.
4. Add direct public exports to feature barrels for all migrated functions used outside the owner.
...
4. Export query options and function types from each owning feature barrel.
...
4. Add a focused test for each public query function’s request path or failure behavior.
...
- Modify: all remaining `frontend/src/**/*.{ts,tsx}` imports matching `@/lib/*.functions`.
- Modify: `frontend/src/hooks/index.ts`, feature barrels, and `frontend/src/types/index.ts` as needed.

1. Search for `@/lib/.*\.functions` and classify every result: it must either be a retained shared utility with a documented reason or be migrated.
2. Search for deep feature imports (`@/features/<feature>/components/...`) across unrelated features and replace them with the owner’s barrel where a public export exists.
...
3. Commit in logical batches, for example:
...
4. Do not mix page-component decomposition or runtime behavior changes into these commits.
