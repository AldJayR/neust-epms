...
export function AdminSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const authenticatedMatch = routerState.matches.find((m) => m.routeId === '/_authenticated')
  ...
  const fullName = user ? `${user.firstName} ${user.lastName}` : 'Engr. J. Dela Cruz'
              ...
              <SidebarMenuButton size="lg" asChild>
                <Link to="/dashboard" className="flex w-full items-center gap-2">
                  <div className="flex aspect-square size-8 items-center justify-center">
                    <img src="https://www.figma.com/api/mcp/asset/90ecf408-1ada-46b9-91dc-9ebc2c099802" alt="NEUST Logo" className="size-7" />
                  </div>
      ...
      <SidebarContent>
        {navMain.map((group) => (
          <SidebarGroup key={group.title}>
            <SidebarGroupLabel className="text-[#0a0a0a] opacity-70">{group.title}</SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  ...
                  <div className="grid flex-1 text-left text-sm leading-tight">
                    <span className="truncate font-semibold text-[#0a0a0a]">{fullName}</span>
                    <span className="truncate text-xs text-[#0a0a0a]">{emailOrRole}</span>
                  </div>
