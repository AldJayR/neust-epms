...
export const getAdminStatsFn = createServerFn({ method: 'GET' }).handler(
  async () => {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorBody.error?.message ?? 'Failed to fetch admin stats')
    }
...
export const bulkUpdateUserStatusFn = createServerFn({ method: 'PATCH' })
  ...
  .handler(async ({ data }) => {
    ...
    if (!response.ok) {
      const errorBody = (await response.json()) as ApiErrorResponse
      throw new Error(errorBody.error?.message ?? 'Failed to update user status')
    }

    return (await response.json()) as { success: boolean; updatedCount: number }
  })
